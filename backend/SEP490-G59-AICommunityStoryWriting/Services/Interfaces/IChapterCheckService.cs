...
public interface IChapterCheckService
{
    ...
    Task<CheckChapterResponse> CheckBannedWordsOnlyAsync(CheckChapterBannedWordsRequest request, Guid? userId, CancellationToken cancellationToken = default);

    /// <summary>Chỉ kiểm tra chính tả (không chạy từ cấm). Dùng cho đồng sáng tác sau khi đã kiểm tra từ cấm riêng.</summary>
    Task<CheckChapterResponse> CheckSpellingOnlyAsync(CheckChapterSpellingRequest request, Guid? userId, CancellationToken cancellationToken = default);
}
